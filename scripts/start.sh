...
docker run -d --name web --link db:db --link cache:cache --volumes-from workspace wrkfree/web node /workspace/wrkfree2.0/webServer/server/server.js

echo "start socket service ... "
docker run -d --name ws --link db:db --link cache:cache --volumes-from workspace wrkfree/ws node /workspace/wrkfree2.0/socketServer/server.js
